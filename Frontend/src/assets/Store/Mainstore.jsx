...
export const moviesApi = createAsyncThunk(
  ...
  async (query, thunkApi) => {
    ...
    const res = await axios.get(`${env.VITE_BASE_URL}${queryurl}`, { headers });
...
export const userSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder.addCase(userApi.fulfilled, (state, action) => {
      ...
      state.user =
        action.payload.data.usercreated || action.payload.data.userfound;
